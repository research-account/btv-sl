...
class TransformerDecoder(Decoder):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self._output_size = vocab_size
        self.bayesian = bayesian_attention or bayesian_feedforward or bayesian_output
        if self.bayesian:
        ...
        if bayesian_output:
            self.output_layer = DenseBayesian(input_features=hidden_size, output_features=vocab_size,
                             competitors = 1, activation = 'linear',prior_mean=0, prior_scale=1. , kl_w=0.001,ibp = ibp)
        else:
            self.output_layer = nn.Linear(hidden_size, vocab_size, bias=False)
    ...
    def forward(
        ...
        **kwargs
        ...
        if self.training:
            return self.forward_(trg_embed,encoder_output,encoder_hidden,src_mask,unroll_steps,hidden,trg_mask,**kwargs)
      
        else:
            
            output    = 0
            x         = 0
            embed_s   = trg_embed.shape[-1]
            encoder_s = encoder_output.shape[-1]
            
            inference_sample_size= max(self.inference_sample_size,embed_s,encoder_s)
            
            for i in range(inference_sample_size):
               
               output_, x_, _,_ =  self.forward_(trg_embed[...,i%embed_s],encoder_output[...,i%encoder_s],
        ...
        x=x*1.0/inference_sample_size
        
        return output, x, None, None
 
   # forward hidden (single run)             
    def forward_(
        self,
        trg_embed: Tensor = None,
