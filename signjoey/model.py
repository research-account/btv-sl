...
from signjoey.utils import parameterConstraints, model_kl_divergence_loss

from signjoey.layers import DenseBayesian

from signjoey.EnsembleTransformer import *
...
def build_model(
    ...
    gloss_input=cfg.get("gloss_input", False)
    DenseBayesian.simplified_inference=cfg.get("simplified_inference",True)
    
    
    if gloss_input:
        if ensemble:
            sgn_embed: Union[Embeddings, None] = EnsembleEmbeddings(
                    N=ensembleN,
            ...
            sgn_embed: Union[Embeddings, None] = Embeddings(
                    **cfg["encoder"]["embeddings"],
                    num_heads=cfg["encoder"]["num_heads"],
                    vocab_size=len(gls_vocab),
                    padding_idx=gls_padding_idx,
       ...
       if ensemble:
            sgn_embed: SpatialEmbeddings = EnsembleSpatialEmbeddings(
               N=ensembleN,
                **cfg["encoder"]["embeddings"],
                num_heads=cfg["encoder"]["num_heads"],
                input_size=sgn_dim,
    ...
    if cfg["encoder"].get("type", "recurrent") == "transformer":
        ...
        if ensemble:
            encoder = EnsembleTransformerEncoder(
                N=ensembleN,
                **cfg["encoder"],
    ...
    if do_translation:
        if ensemble:
            txt_embed: Union[Embeddings, None] = EnsembleEmbeddings(
                N=ensembleN,
        ...
        if cfg["decoder"].get("type", "recurrent") == "transformer":
            if ensemble:
                decoder = EnsembleTransformerDecoder(
                N=ensembleN,
                **cfg["decoder"],
                encoder=encoder,
                vocab_size=len(txt_vocab),
    ...
    for layer in DenseBayesian.instances:
