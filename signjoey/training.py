...
from signjoey.utils import model_kl_divergence_loss 
#from signjoey.transformer_layers import globloss
...
class TrainManager:
    ...
    def __init__(self, model: SignModel, config: dict) -> None:
        ...
        self.do_kl = (
            config["training"].get("kl_weight", 0.0) > 0.0
        )
    ...
    def train_and_validate(self, train_data: Dataset, valid_data: Dataset) -> None:
        ...
        for epoch_no in range(self.epochs):
            ...
            for batch in iter(train_iter):
                ...
                recognition_loss, translation_loss,kl_loss = self._train_batch(
                    batch, update=update
                )
                ...
                if self.steps % self.logging_freq == 0 and update:
                    ...
                    if self.do_kl:
                        ...
                        processed_txt_tokens = self.total_txt_tokens
                        log_out += "Batch KL Loss: {:10.6f} || ".format(
                            kl_loss
                        )
    ...
    def _train_batch(self, batch: Batch, update: bool = True) -> (Tensor, Tensor):
        ...
        #kl_loss =0
        #kls=MultiHeadedAttention.kls*0.1
        

        if self.model.do_kl:
            kl_loss = self.model.kl_loss(self.model,self.model.kl_weight)

     
        total_loss = normalized_recognition_loss + normalized_translation_loss+kl_loss
        ...
        MultiHeadedAttention.kls=0
        return normalized_recognition_loss, normalized_translation_loss,kl_loss
...
def train(cfg_file: str) -> None:
    ...
    del trainer
    test(cfg_file, ckpt=ckpt, output_path=output_path, logger=logger,bit=False)
    test(cfg_file, ckpt=ckpt, output_path=output_path, logger=logger,bit=True)
