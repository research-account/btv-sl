...
import pandas as pd
import numpy as np
from signjoey.local_attention import LocalAttention
...
class MultiHeadedAttention(nn.Module):
    ...
    def __init__(self, num_heads: int, size: int, dropout: float = 0.1,bayesian=False,ibp=False,sizek=None,scale_out=1.0):
        ...
        self.printcounter=0
        self.pe=PositionalEncoding(size)
...
class PositionwiseFeedForward(nn.Module):
    ...
    def __init__(self, input_size, ff_size, dropout=0.1,bayesian=False,ibp=False,activation='relu',lwta_competitors=4,scale_out=0.2):
        """
        ...
        if bayesian:
            ...
            self.pwff_layer = nn.Sequential(
               DenseBayesian(input_size , ff_size,
                           competitors = lwta_competitors, activation = activation, prior_mean=0, prior_scale=1. , kl_w=0.01,ibp = ibp),
               nn.Dropout(dropout),
              
               DenseBayesian(input_features=ff_size, output_features=input_size,
                            competitors = 1, activation = 'linear',prior_mean=0, prior_scale=1. , ibp = ibp,out_w=False,scale_out=scale_out),
...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        self.trg_trg_att = MultiHeadedAttention(num_heads, size, dropout=dropout,
            bayesian=bayesian_attention,ibp=ibp)
       
        
        self.src_trg_att = MultiHeadedAttention(num_heads, size, dropout=dropout,
            bayesian=bayesian_attention,ibp=ibp,sizek=size,scale_out=1.0)        
        self.feed_forward = PositionwiseFeedForward( input_size=size, ff_size=ff_size, dropout=dropout,
            bayesian=bayesian_feedforward,ibp=ibp,activation=activation,lwta_competitors=lwta_competitors
        )
