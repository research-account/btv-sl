...
def test(
    cfg_file, ckpt: str, output_path: str = None, logger: logging.Logger = None,bit=False
) -> None:
    """
    ...
    if use_cuda:
        model.cuda()
    if bit:
        layers_list=get_children(model)
        layers=[]
        IDs=[]
        for layer in layers_list:

            if hasattr(layer, 'posterior_mean') and  hasattr(layer, 'ID') and not layer.ID in IDs :
                layers.append(layer)
                IDs.append(layer.ID)
     
       # thresholds = FLAGS.thresholds
        compute_compression_rate(layers,None)

        print("Test error  with reduced bit precision:")

        weights = compute_reduced_weights(layers,None)
        for layer, weight in zip(layers, weights):
            if use_cuda:
                layer.posterior_mean.data = torch.Tensor(weight).cuda()
            else:
                layer.posterior_mean.data = torch.Tensor(weight)
